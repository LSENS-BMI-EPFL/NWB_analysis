...
sys.path.append('/Users/nigro/Desktop/allen_utils')
        ...
        if (beh_type == 'whisker') and (day != 0):
            ...
            for secondary_target in secondary_targets:
                ...
                if electrode_table is None:
                    ...
                    continue
                electrode_table = convert_electrode_group_object_to_columns(electrode_table) # format the electrode_group column into something readable
                electrode_table= convert_elec_table_col_names_for_ripples(electrode_table)
                electrode_table['mouse_id'] = mouse # add the mouse_id for the allen labels functions
                electrode_table = allen_utils.process_allen_labels(electrode_table, subdivide_areas=True)
                

                is_ca1 = ripple_target in electrode_table.area_acronym_custom.unique()
                is_secondary_target_names = len([i for i in electrode_table.area_acronym_custom.unique() if secondary_target in i]) > 0
                ...
                print(f'Location : {electrode_table.area_acronym_custom.unique()}')
                ripple_stream = int(electrode_table.loc[electrode_table.area_acronym_custom == ripple_target].group_name.values[0][-1])
                secondary_steam = int(electrode_table.loc[electrode_table.area_acronym_custom.str.startswith(secondary_target)].group_name.values[0][-1])
                ...
                units_df['mouse_id'] = mouse # add the mouse_id for the allen labels functions

                # we add a new column for the location of each unit 
                units_df = convert_electrode_group_object_to_columns(units_df) # format the electrode_group column into something readable
                units_df = allen_utils.process_allen_labels(units_df, subdivide_areas=True)

                print(f'regions in units table : {units_df["area_acronym_custom"].unique()}')
