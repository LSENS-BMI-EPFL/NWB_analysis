...
def compute_corr_numpy(template, target):

    r = []
    for trial in tqdm(range(template.shape[0])):
            r += [[correlate(template[trial, :], target[px, trial, :])[1, 0] for px in range(target.shape[0])]]
...
def trial_based_correlation(mouse_id, session_id, trial_table, dict_roi, data_roi, data_frames):
    ...
    for roi in dict_roi.keys():
        ...
        # Correlate data
        corr = compute_corr_numpy(template, target)
        trial_table[f'{roi}_r'] = corr

        # Shuffle all trials
        all_trial_shuffle = []
        for i in range(50):
            shuffle = np.random.permutation(np.arange(template.shape[0]))
            all_trial_shuffle += [compute_corr_numpy(template[shuffle], target)]
        ...
        for i in range(50):
            ...
            shuffle = np.hstack([np.where(block_id == block)[[0]] for block in np.random.permutation(np.unique(block_id))])[0]
            block_shuffle += [compute_corr_numpy(template[shuffle], target)]
        ...
        for i in range(50):
            ...
            within_block_shuffle += [compute_corr_numpy(template[shuffle], target)]
...
if __name__ == '__main__':
    ...
    for state in ['naive', 'expert']:
        if os.environ['COMPUTERNAME'] == 'SV-07-097':
            ...
            main([file], result_path=result_path, correct_trials=False)