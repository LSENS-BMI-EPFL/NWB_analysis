...
def trialbased_logregress_model(X, y_binary, result_path):
    ...
    trial_based_accuracy.to_json(os.path.join(result_path, 'model_trial_based_scores.json'))

    avg_results = pd.concat([pd.DataFrame(res) for res in avg_results])
    avg_results.to_json(os.path.join(result_path, 'model_results.json'))
    ...
    trial_based_accuracy = pd.concat([pd.DataFrame(res) for res in trial_based_accuracy])
    trial_based_accuracy.to_json(os.path.join(result_path, 'null_trial_based_scores.json'))

    avg_results = pd.concat([pd.DataFrame(res) for res in avg_results])
    avg_results.to_json(os.path.join(result_path, 'null_results.json'))
