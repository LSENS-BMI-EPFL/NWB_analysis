...
def logregress_classification(nwb_files, classify_by, output_path):
    ...
    for nwb_file in nwb_files:
        ...
        if classify_by == 'context':
            ...
            y_binary += trial_table.loc[trial_table.correct_choice, 'lick_flag']
...
if __name__ == "__main__":
    ...
    data_path = r"/scratch/bechvila/NWB"

    if not os.path.exists(output_path):
        os.makedirs(output_path)

    nwb_files = os.listdir(data_path)
    nwb_files = [file for file in nwb_files if mouse_id in file]

    print(" ")
    print(f"Analyzing {len(nwb_files)} sessions from mouse : {mouse_id}")
